...
public class Constant implements iDataFilter {
	public static boolean DataError;

	/**
	 * If the value of measured data remains the same, there is an error and the
	 * method will return a map with NaN values where errors were found. If the
	 * method finds an error, it will set the static boolean DataError to true.
	 * If not, the DataError value is set to be false.
	 * 
	 * @param data
	...
	public SortedMap<Date, Float> check(SortedMap<Date, Float> data,
		...
		for (Entry<Date, Float> entry : data.entrySet()) {
			if (index >= neighbour) {
				if (constantDetection(neighbours, deviation)) {
	...
	/**
	 * This method is used to detect if there is a constant error occurring.
	 * 
	 * @param neighbours
	 * @param deviation
	 * @return true error found / false - no error
	 */
	private boolean constantDetection(SortedMap<Date, Float> neighbours,
			float deviation) {
